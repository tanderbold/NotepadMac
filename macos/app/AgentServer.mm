    ...
    [self addTool:@"compare"
      description:@"Compares two texts line by line as the editor's Compare does (Myers' algorithm): each side is an "
        ...
        if (BoolParam(args, @"show", NO)) {
            ...
            // The options are the user's preferences: shown with the agent's,
            // then given back, so a call does not change the user's settings.
            BOOL wasCase = ed.compareIgnoreCase, wasSpaces = ed.compareIgnoreSpaces, wasEmpty = ed.compareIgnoreEmptyLines;
            ...
            ed.compareIgnoreCase = wasCase;
            ed.compareIgnoreSpaces = wasSpaces;
            ed.compareIgnoreEmptyLines = wasEmpty;
        }
        return out;
    }];
