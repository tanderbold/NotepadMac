...
- (NSUInteger)appendMatchesOf:(NppRegex *)regex inFile:(NSString *)path contents:(NSString *)contents
    ...
    // ProcessFindAll's empty matches (SCFIND_REGEXP_EMPTYMATCH_NOTAFTERMATCH), as Find All's.
    [regex enumerateMatchesWithGroupsInData:data range:NSMakeRange(0, length) emptyMatches:NppEmptyMatchesNotAfterMatch
                                 usingBlock:^(NSArray<NSValue *> *groups, BOOL *stop) {
        NSRange match = groups.firstObject.rangeValue;
        if (search.cancelled) { *stop = YES; return; }
